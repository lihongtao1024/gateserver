...
func (proto *RandKeyNtf) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteIntegerArray(b, proto.Code_content, uint8(255)) {
		return false
	}
...
func (proto *RandKeyNtf) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadIntegerArray(b, &proto.Code_content, uint8(255)) {
		return false
	}
...
func (proto *LoginReq) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteInteger(b, proto.Ip) {
		return false
	}

	if !ProtoWriteIntegerArray(b, proto.Pwd_content, uint8(255)) {
		return false
	}

	if !ProtoWriteString(b, proto.Hwid, 256) {
		return false
	}

	if !ProtoWriteInteger(b, proto.Client_type) {
		return false
	}

	if !ProtoWriteString(b, proto.Client_version, 256) {
		return false
	}

	if !ProtoWriteString(b, proto.Client_protocol_md5, 256) {
		return false
	}

	if !ProtoWriteString(b, proto.Longitude, 64) {
		return false
	}

	if !ProtoWriteString(b, proto.Latitude, 64) {
		return false
	}

	if !ProtoWriteInteger(b, proto.Session) {
		return false
	}

	if !ProtoWriteInteger(b, proto.Suid) {
		return false
	}
...
func (proto *LoginReq) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadInteger(b, &proto.Ip) {
		return false
	}

	if !ProtoReadIntegerArray(b, &proto.Pwd_content, uint8(255)) {
		return false
	}

	if !ProtoReadString(b, &proto.Hwid, 256) {
		return false
	}

	if !ProtoReadInteger(b, &proto.Client_type) {
		return false
	}

	if !ProtoReadString(b, &proto.Client_version, 256) {
		return false
	}

	if !ProtoReadString(b, &proto.Client_protocol_md5, 256) {
		return false
	}

	if !ProtoReadString(b, &proto.Longitude, 64) {
		return false
	}

	if !ProtoReadString(b, &proto.Latitude, 64) {
		return false
	}

	if !ProtoReadInteger(b, &proto.Session) {
		return false
	}

	if !ProtoReadInteger(b, &proto.Suid) {
		return false
	}
...
func (proto *LoginAck) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteInteger(b, proto.Rid) {
		return false
	}
...
func (proto *LoginAck) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadInteger(b, &proto.Rid) {
		return false
	}
...
func (proto *ReloginReq) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteString(b, proto.Hwid, 128) {
		return false
	}
...
func (proto *ReloginReq) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadString(b, &proto.Hwid, 128) {
		return false
	}
...
func (proto *ReloginAck) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteInteger(b, proto.Gsindex) {
		return false
	}

	if !ProtoWriteInteger(b, proto.Arrayid) {
		return false
	}
...
func (proto *ReloginAck) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadInteger(b, &proto.Gsindex) {
		return false
	}

	if !ProtoReadInteger(b, &proto.Arrayid) {
		return false
	}
...
func (proto *PlayerListAck) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteInteger(b, proto.Lastplayer) {
		return false
	}

	if !ProtoWriteCustomArray(b, proto.Briefs, uint8(255)) {
		return false
	}
...
func (proto *PlayerListAck) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadInteger(b, &proto.Lastplayer) {
		return false
	}

	if !ProtoReadCustomArray(b, &proto.Briefs, uint8(255)) {
		return false
	}
...
func (proto *CreatePlayerReq) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteCustom(b, &proto.Playerbrief) {
		return false
	}
...
func (proto *CreatePlayerReq) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadCustom(b, &proto.Playerbrief) {
		return false
	}
...
func (proto *TrdLoginReq) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteString(b, proto.Platform, 255) {
		return false
	}

	if !ProtoWriteIntegerArray(b, proto.Token, uint16(65535)) {
		return false
	}
...
func (proto *TrdLoginReq) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadString(b, &proto.Platform, 255) {
		return false
	}

	if !ProtoReadIntegerArray(b, &proto.Token, uint16(65535)) {
		return false
	}
...
func (proto *GetWSTimestampAck) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteInteger(b, proto.Zone) {
		return false
	}
...
func (proto *GetWSTimestampAck) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadInteger(b, &proto.Zone) {
		return false
	}
...
func (proto *RealnameInfoNtf) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteInteger(b, proto.Realname_status) {
		return false
	}

	if !ProtoWriteString(b, proto.Auth, 64) {
		return false
	}

	if !ProtoWriteString(b, proto.Realname_token, 64) {
		return false
	}
...
func (proto *RealnameInfoNtf) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadInteger(b, &proto.Realname_status) {
		return false
	}

	if !ProtoReadString(b, &proto.Auth, 64) {
		return false
	}

	if !ProtoReadString(b, &proto.Realname_token, 64) {
		return false
	}
