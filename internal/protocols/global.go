...
func (proto *ServerConfigNtf) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteInteger(b, proto.Svr_id) {
		return false
	}

	if !ProtoWriteInteger(b, proto.Limit) {
		return false
	}

	if !ProtoWriteCustomArray(b, proto.Listen, uint8(255)) {
		return false
	}

	if !ProtoWriteCustomArray(b, proto.Connect, uint8(255)) {
		return false
	}
...
func (proto *ServerConfigNtf) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadInteger(b, &proto.Svr_id) {
		return false
	}

	if !ProtoReadInteger(b, &proto.Limit) {
		return false
	}

	if !ProtoReadCustomArray(b, &proto.Listen, uint8(255)) {
		return false
	}

	if !ProtoReadCustomArray(b, &proto.Connect, uint8(255)) {
		return false
	}
...
func (proto *DBWrapperPkg) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteIntegerArray(b, proto.Wrapper, uint32(4294967295)) {
		return false
	}
...
func (proto *DBWrapperPkg) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadIntegerArray(b, &proto.Wrapper, uint32(4294967295)) {
		return false
	}
...
func (proto *LoadAuctionObjectDataAck) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteInteger(b, proto.Total_num) {
		return false
	}

	if !ProtoWriteInteger(b, proto.Remain_num) {
		return false
	}

	if !ProtoWriteCustomArray(b, proto.Datas, uint32(4294967295)) {
		return false
	}
...
func (proto *LoadAuctionObjectDataAck) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadInteger(b, &proto.Total_num) {
		return false
	}

	if !ProtoReadInteger(b, &proto.Remain_num) {
		return false
	}

	if !ProtoReadCustomArray(b, &proto.Datas, uint32(4294967295)) {
		return false
	}
...
func (proto *LoadRanklistDataAck) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteCustomArray(b, proto.Data, uint32(4294967295)) {
		return false
	}
...
func (proto *LoadRanklistDataAck) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadCustomArray(b, &proto.Data, uint32(4294967295)) {
		return false
	}
...
func (proto *FregmentPkg) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteInteger(b, proto.Frag_mark) {
		return false
	}

	if !ProtoWriteIntegerArray(b, proto.Frag_data, uint32(4294967295)) {
		return false
	}
...
func (proto *FregmentPkg) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadInteger(b, &proto.Frag_mark) {
		return false
	}

	if !ProtoReadIntegerArray(b, &proto.Frag_data, uint32(4294967295)) {
		return false
	}
...
func (proto *LoadOfflinePlayerReq) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteInteger(b, proto.Object_type) {
		return false
	}
...
func (proto *LoadOfflinePlayerReq) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadInteger(b, &proto.Object_type) {
		return false
	}
...
func (proto *LoadOfflinePlayerAck) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteCustom(b, &proto.Offline_data) {
		return false
	}
...
func (proto *LoadOfflinePlayerAck) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadCustom(b, &proto.Offline_data) {
		return false
	}
...
func (proto *SyncViolateNtf) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteCustomArray(b, proto.Violates, uint16(65535)) {
		return false
	}
...
func (proto *SyncViolateNtf) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadCustomArray(b, &proto.Violates, uint16(65535)) {
		return false
	}
...
func (proto *SyncForbidTalkNtf) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteCustomArray(b, proto.Forbids, uint32(4294967295)) {
		return false
	}
...
func (proto *SyncForbidTalkNtf) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadCustomArray(b, &proto.Forbids, uint32(4294967295)) {
		return false
	}
...
func (proto *BillQueryReq) Write(b *bytes.Buffer) bool {
	...
	if !ProtoWriteInteger(b, proto.Start_time) {
		return false
	}
	...
	if !ProtoWriteString(b, proto.Callback, 255) {
		return false
	}
...
func (proto *BillQueryReq) Read(b *bytes.Buffer) bool {
	...
	if !ProtoReadInteger(b, &proto.Start_time) {
		return false
	}
	...
	if !ProtoReadString(b, &proto.Callback, 255) {
		return false
	}
