...
func (impl *virtualImpl) loadJson() bool {
	...
	if err != nil {
		singleton.LogInstance.Err("load \"%s\" fail, errmsg: \"%s\"", jsonFile, err)
		return false
	}
	defer file.Close()
...
func (impl *virtualImpl) PostRequest(client component.Client) {
	...
	client.SetUid(uid)
	singleton.VerifyInstance.ReceiveResponse(client, client.GetSuid(), pkg.ErrorOk)
}

func (impl *virtualImpl) ReceiveResponse(client component.Client, err pkg.ErrorCode) {
...
func (impl *virtualImpl) Close() {
	...
	if err != nil {
		singleton.LogInstance.Err("save \"%s\" fail, errmsg: \"%s\"", jsonFile, err)
		return
	}
