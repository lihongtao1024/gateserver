...
func NewComponent() pkg.TimerComponent {
	comp := &timerComponent{
...
func (comp *timerComponent) onTimerImpl(timer *timerImpl) {
	...
	if timer.tmrCount == pkg.InfiniteTimer {
		timer.tmrDispatcher.OnTimer()
	} else {
		timer.tmrCount--
		timer.tmrDispatcher.OnTimer()
	}
...
func (comp *timerComponent) AddTimer(dispatcher pkg.TimerDispatcher,
	interval uint32, count uint32) pkg.Timer {
	if interval == 0 {
		interval = 1
	}
...
func (comp *timerComponent) ModifyTimer(t pkg.Timer,
	dispatcher pkg.TimerDispatcher, interval uint32, count uint32) bool {
	timer := t.(*timerImpl)
	comp.DelTimer(t)
	...
	timer.tmrDispatcher = dispatcher
	comp.addTimerImpl(timer)
	return true
}

func (comp *timerComponent) DelTimer(t pkg.Timer) {
	timer := t.(*timerImpl)
	timer.tmrCount = 0
